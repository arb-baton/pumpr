...
if (dom.terminal) {
  ...
  dom.terminal.addEventListener("keydown", (event) => {
    ...
    const lineHeight = 12; // slightly smaller line height for even finer control
    const pageScroll = Math.floor(el.clientHeight * 0.6); // smaller page scroll for smoother navigation
    let handled = false;
    switch (event.key) {
      case "ArrowDown":
