...
if (dom.terminal) {
  ...
  dom.terminal.addEventListener("keydown", (event) => {
    ...
    if (!el) return;
    const lineHeight = 16; // slightly smaller line height for smoother scroll
    const pageScroll = Math.floor(el.clientHeight * 0.6); // smaller page scroll for better control
    let handled = false;
    switch (event.key) {
      case "ArrowDown":
      case "Down":
